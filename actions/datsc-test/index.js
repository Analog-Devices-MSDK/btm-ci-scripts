...
const runDatscTest = function (targetServer, targetClient, elfServer, elfClient) {
    options = {
        mode: 'text',
        pythonPath: 'python3',
        pythonOptions: ['-u'],
    ...
    return new Promise((resolve, reject) => {
        let pyshell = new PythonShell('datsc_connected.py', options);
...
const main = async function() {
    ...
    let projectServer = path.join(MSDK_PATH, 'Examples', targetServer, 'Bluetooth', 'BLE_dats');
    let projectClient = path.join(MSDK_PATH, 'Examples', targetClient, 'Bluetooth', 'BLE_datc');
    await Promise.all([
    ...
    retCode = await runDatscTest(targetServer, targetClient, elfServer, elfClient).then(
        (success) => procSuccess(success, 'DATSC Test'),
        (error) => procFail(error, 'DATSC Test', false)
    );
    if (retCode === 0) {
